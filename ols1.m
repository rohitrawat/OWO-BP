function [E W Ei NLin oLin] = ols1(R, C, Et)
...
% Updated: 10/23/2013
% removed: error('The constant input found to be dependent!'); % removed  
...
if(g < eps2)    % check for numerical stability (note: why does it imply a dependent basis?)
    ...
    NLin = NLin+1;
    oLin(NLin) = 1;
%     error('The constant input found to be dependent!'); % removed
%     10/23/2013
else
    g = sqrt(g);
    A(1,1) = 1 / g;
...
if(Idebug > 0 && NLin > 0)
    fprintf('OLS1: # of dependent basis functions = %d\n', NLin);
    fprintf('OLS1: # of basis functions = %d\n', Nu);
%     pause;
end
